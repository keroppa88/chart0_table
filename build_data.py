...
def load_chartlist():
    """allchart.csv / allchartlist.csv からコード→銘柄名のマッピングを読み込む。"""
    mapping = {}
    loaded_files = []

    for filepath in CHARTLIST_FILES:
    ...
    if not loaded_files:
        ...
        print(f"  銘柄名マッピング読込: {', '.join(loaded_files)}（後勝ちで上書き）")
    else:
        print(f"  銘柄名マッピング読込: {loaded_files[0]}")
